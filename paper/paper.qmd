...
# Summarize the data by incident type (combining bus and streetcar delays)
incident_summary <- combined_data %>%
  group_by(Incident) %>%
  summarise(Total_Incidents = n()) %>%
  arrange(desc(Total_Incidents))

# Plot the total incidents for each incident type using ggplot2
...
# Summarize the data by Delay.Group
...
# Plot the total incidents for each delay group using ggplot2
...
file1 <- ("ttc_subway_delay_data_2024.csv")
file2 <- ("ttc_bus_delay_data_2024.csv")
file3 <- ("ttc_streetcar_delay_data_2024.csv")
...
# Assuming the date column might be named 'Date'
data1$date <- as.Date(data1$Date, format = "%Y-%m-%d")
data2$date <- as.Date(data2$Date, format = "%Y-%m-%d")
data3$date <- as.Date(data3$Date, format = "%Y-%m-%d")

# Extract the day of the week for each file and count incidents
day_counts_file1 <- data1 %>%
  mutate(day_of_week = wday(date, label = TRUE)) %>%
  group_by(day_of_week) %>%
  summarise(total_count = n()) %>%
  mutate(file = "File 1")

day_counts_file2 <- data2 %>%
  mutate(day_of_week = wday(date, label = TRUE)) %>%
  group_by(day_of_week) %>%
  summarise(total_count = n()) %>%
  mutate(file = "File 2")

day_counts_file3 <- data3 %>%
  mutate(day_of_week = wday(date, label = TRUE)) %>%
  group_by(day_of_week) %>%
  summarise(total_count = n()) %>%
  mutate(file = "File 3")
...
combined_day_counts <- bind_rows(day_counts_file1, day_counts_file2, day_counts_file3)

# Plot the variation of delay incidents by day of the week for each file
ggplot(combined_day_counts, aes(x = day_of_week, y = total_count, color = file, group = file)) +
  geom_line() +
  geom_point() +
  labs(title = "Variation of Delay Incidents by Day of the Week",
       x = "Day of the Week",
       y = "Total Count",
       color = "File") +
  theme_minimal()
